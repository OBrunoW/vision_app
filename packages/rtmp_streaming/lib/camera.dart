  ...
  Widget build(BuildContext context) {
    if (controller.value.isInitialized!) {
      ...
      if (Platform.isAndroid) {
        // Hybrid composition: evita virtual display que estica o preview.
        childView = PlatformViewLink(
...
/// Codificador nativo usado no Android (RTMP ou RTSP).
enum StreamEncoder {
  rtmp,
  rtsp,
}
...
class CameraController extends ValueNotifier<CameraValue> {
  CameraController(
    ...
    this.streamEncoder = StreamEncoder.rtmp,
  ...
  /// Android: [StreamEncoder.rtsp] usa RtspCamera2 (RootEncoder).
  final StreamEncoder streamEncoder;
    ...
    try {
      ...
      final Map<String, dynamic> reply =
        ...
        <String, dynamic>{
          ...
          'streamProtocol': streamEncoder.name,
        },
      ))!;
  ...
  /// Troca o codificador nativo (RTMP/RTSP) sem reinicializar o [CameraController].
  Future<void> switchStreamEncoder(StreamEncoder encoder) async {
    if (!value.isInitialized! || _isDisposed) return;
    if (!Platform.isAndroid) return;
    try {
      await _channel.invokeMethod<void>('switchStreamEncoder', <String, dynamic>{
        'streamProtocol': encoder.name,
      });
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  ...
  /// Reinicia o preview após a superfície Android ser recriada (ex.: voltar da minimização).
