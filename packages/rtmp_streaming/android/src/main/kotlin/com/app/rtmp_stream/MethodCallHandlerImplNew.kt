...
//        imageStreamChannel = EventChannel(messenger, "plugins.flutter.io/rtmp_publisher/imageStream")
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "switchStreamEncoder" -> {
                Log.i("Stuff", "switchStreamEncoder")
                val useRtsp = call.argument<String>("streamProtocol") == "rtsp"
                getCameraView()?.switchStreamEncoder(useRtsp, result)
                    ?: result.error("no_camera", "Camera not initialized", null)
            }
            "resumePreview" -> {
                Log.i("Stuff", "resumePreview")
    ...
    private fun instantiateCamera(call: MethodCall, result: MethodChannel.Result) {
        ...
        currentHandler.postDelayed({
            ...
            try {
                ...
                nativeViewFactory?.useRtspEncoder =
                    call.argument<String>("streamProtocol") == "rtsp"
