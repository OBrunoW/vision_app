...
class CameraNativeView(
    ...
    private val useRtspEncoder: Boolean = false,
) :
    PlatformView,
...
//        glView.isKeepAspectRatio = true
        ...
        bitrateAdapter = BitrateAdapter {
            streamCamera.setVideoBitrateOnFly(it)
        }.apply {
            setMaxBitrate(vBitrate + aBitrate)
        }
    }

    private fun createEncoder(useRtsp: Boolean): Camera2Base =
        if (useRtsp) RtspCamera2(glView, this) else RtmpCamera2(glView, this)

    private fun configureStreamEncoder() {
        streamCamera.streamClient.setReTries(10)
        streamCamera.setFpsListener { fps = it }
    }

    fun switchStreamEncoder(useRtsp: Boolean, result: MethodChannel.Result) {
        if (useRtsp == isRtspEncoder) {
            result.success(null)
            return
        }
        try {
            if (streamCamera.isStreaming) streamCamera.stopStream()
            if (streamCamera.isOnPreview) streamCamera.stopPreview()
            isRtspEncoder = useRtsp
            streamCamera = createEncoder(useRtsp)
            configureStreamEncoder()
            if (isSurfaceCreated) startPreview(cameraName)
            result.success(null)
        } catch (e: Exception) {
            result.error("switchStreamEncoder", e.message, null)
        }
    }
    ...
    override fun onNewBitrate(bitrate: Long) {
        bitrateAdapter.adaptBitrate(bitrate, streamCamera.getStreamClient().hasCongestion())
    }

    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
    override fun onConnectionFailed(reason: String) {
        activity?.runOnUiThread { //Wait 5s and retry connect stream
            if (streamCamera.streamClient.reTry(5000, reason)) {
    ...
    fun switchFlashLight(isEnable: Boolean?, result: MethodChannel.Result) {
        try {
            if(streamCamera.cameraFacing != BACK){
            ...
            if(isEnable == true){
                 streamCamera.enableLantern()
            }else{
                streamCamera.disableLantern()
            }
          result.success(null)
        } catch (e: CameraAccessException) {
        ...
        try {
          ...
          when (type) {
            0 -> {
              ...
              streamCamera.glInterface?.setFilter(f)
              currentFilter = f
            ...
            6 -> {
              ...
              streamCamera.glInterface?.setFilter(chromaFilterRender)
            ...
            20 -> {
              ...
              streamCamera.glInterface?.setFilter(gifObjectFilterRender)
            ...
            23 -> {
              ...
              streamCamera.glInterface?.setFilter(imageObjectFilterRender)
            ...
            32 -> {
              ...
              streamCamera.glInterface?.setFilter(rgbSaturationFilterRender)
            ...
            34 -> {
              ...
              streamCamera.glInterface?.setFilter(rotationFilterRender)
            ...
            39-> {
              ...
              streamCamera.glInterface?.setFilter(surfaceFilterRender)
            ...
            41 -> {
              ...
              streamCamera.glInterface?.setFilter(textObjectFilterRender)
    ...
    fun removeFilter(type: Int?, result: MethodChannel.Result) {
        try {
          ...
          if (filterToRemove != null && filterType == type) {
            streamCamera.glInterface?.removeFilter(filterToRemove)
            currentFilter = null
...
//                Log.d("error", targetCamera)
    ...
    fun getStreamStatistics(result: MethodChannel.Result) {
        ...
        ret["cacheSize"] = streamCamera.streamClient.getCacheSize()
        ret["sentAudioFrames"] = streamCamera.streamClient.getSentAudioFrames()
        ret["sentVideoFrames"] = streamCamera.streamClient.getSentVideoFrames()
        ret["droppedAudioFrames"] = streamCamera.streamClient.getDroppedAudioFrames()
        ret["droppedVideoFrames"] = streamCamera.streamClient.getDroppedVideoFrames()
        ret["bytesSend"] = streamCamera.streamClient.getBytesSend()
        ret["isAudioMuted"] = streamCamera.isAudioMuted
        ret["bitrate"] = streamCamera.bitrate
        ret["width"] = streamCamera.streamWidth
        ret["height"] = streamCamera.streamHeight
        ret["fps"] = fps
        val rtmpSc = streamCamera.streamClient as? RtmpStreamClient
    ...
    fun setForceBt709Color(enabled: Boolean?, result: MethodChannel.Result) {
        ...
        try {
            streamCamera.forceBt709Color(enabled)
            result.success(null)
        } catch (e: Exception) {
